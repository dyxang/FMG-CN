...
window.BurgsAndStates = (function () {
  const generate = function () {
    ...
    function placeCapitals() {
      ...
      if (sorted.length < count * 10) {
        ...
        if (!count) {
          WARN && console.warn("没有填充的单元格。无法生成国家");
          return burgs;
        } else {
          WARN && console.warn(`人数不够 (${sorted.length}). 将只生成 ${count} 国家`);
        }
      }
      ...
      for (let i = 0; burgs.length <= count; i++) {
        ...
        if (i === sorted.length - 1) {
          WARN && console.warn("无法使用当前间距放置首都。请使用减小的间距重试");
  ...
  const generateDiplomacy = function () {
    ...
    for (let attacker = 1; attacker < states.length; attacker++) {
      ...
      // start a war
      const war = [`${an}-${trimVowels(dn)}ian War`, `${an} 向对手 ${dn} 宣战`];
      ...
      ad.forEach((r, d) => {
        if (r === "Suzerain") {
          attackers.push(d);
          war.push(`${an} 的附庸 ${states[d].name} 参加了进攻方的战争`);
        }
      });
      ...
      dd.forEach((r, d) => {
        if (r === "Suzerain") {
          defenders.push(d);
          war.push(`${dn} 的附庸 ${states[d].name} 参加了防守方的战争`);
        }
      });
      ...
      dd.forEach((r, d) => {
        ...
        if (states[d].diplomacy[attacker] !== "Rival" && ap / dp > 2 * gauss(1.6, 0.8, 0, 10, 2)) {
          const reason = states[d].diplomacy.includes("Enemy") ? "已经在打仗了," : `被 ${an} 吓到了,`;
          war.push(`${reason} ${states[d].name} 解除了与 ${dn} 的防御协定`);
          dd[d] = states[d].diplomacy[defender] = "Suspicion";
          return;
        }
        defenders.push(d);
        dp += states[d].area * states[d].expansionism;
        war.push(`${dn} 的盟友 ${states[d].name} 参加了防守方的战争`);
        ...
        states[d].diplomacy
          ...
          .forEach(v => {
            ...
            war.push(`${states[d].name} 的盟友 ${states[v].name} 参加了防守方的战争`);
          });
      });
      ...
      ad.forEach((r, d) => {
        ...
        states[d].diplomacy
          ...
          .forEach(v => {
            attackers.push(v);
            dp += states[v].area * states[v].expansionism;
            war.push(`${states[d].name} 的附庸 ${states[v].name} 参加了进攻方的战争`);
          });
      });
