...
function generateMapWithSeed() {
  if (optionsSeed.value == seed) return tip("当前的地图已经有了这个种子", false, "error");
  regeneratePrompt();
}
...
function showSeedHistoryDialog() {
  const lines = mapHistory.map((h, i) => {
    ...
    const button = `<i data-tip="单击此处生成带有此种子的地图" onclick="restoreSeed(${i})" class="icon-history optionsSeedRestore"></i>`;
...
async function enter3dView(type) {
  ...
  canvas.onmouseenter = () => {
    const help =
      "左鼠标改变角度，中鼠标键/鼠标滚轮缩放，右鼠标平移. 按<b>O</b>来切换选项";
