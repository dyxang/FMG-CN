...
function editNamesbase() {
  ...
  function analyzeNamesbase() {
    ...
    const getLengthQuality = () => {
      if (length < 30) return "<span data-tip='名称库包含小于30个名称——不足以生成合理的数据' style='color:red'>[not enough]</span>";
      if (length < 100) return "<span data-tip='名称库包含小于100个名称——不足以生成好的名称' style='color:darkred'>[low]</span>";
      if (length <= 400) return "<span data-tip='名称库包含合理数量的示例的命名 style='color:green'>[good]</span>";
      return "<span data-tip='名称库包含大于400个名称。这太多了，尽量减少到大约300个名称' style='color:darkred'>[overmuch]</span>";
    };
  ...
  function namesbaseAdd() {
    ...
    document.getElementById("namesbaseExamples").innerHTML = "请提供姓名数据";
  }
