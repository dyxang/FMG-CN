...
function editEmblem(type, id, el) {
  ...
  function upload(type) {
    ...
    if (file.size > 500000) {
      tip(`文件太大，请优化文件大小高达500kB，并重新上传。建议大小为200x200像素，最大可达100kB`, true, "error", 5000);
      return;
    }
    ...
    reader.onload = function (readerEvent) {
      ...
      if (type === "image") {
        ...
        if (!svg) {
          tip("该文件应准备加载到 FMG。请使用 Armoria 或其他相关工具", false, "error");
          return;
        }
  ...
  async function downloadGallery() {
    ...
    function runDownload() {
      ...
      const html = /* html */ `<!DOCTYPE html>
        <html>
          <head>
            <title>${mapName.value} 徽标画廊</title>
          </head>
          <style type="text/css">
            body {
              margin: 0;
          ...
          <body>
            ...
            <address>由 ${FMG} 生成. 该工具是免费的，但图像可能受版权保护, 查看 ${license}</address>
          </body>
        </html>`;
