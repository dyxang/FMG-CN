  ...
  const generate = function () {
    ...
    if (populated.length < count * 25) {
      ...
      if (!count) {
        ...
        WARN && console.warn(`单元格人数不够 (${populated.length}). 将只生成 ${count} 文化`);
        alertMessage.innerHTML = /* html */ ` 只有 ${populated.length} 人口密集的单元，居住面积不足.<br />
          将只生成 ${culturesInput.value} 个请求的文化中的 ${count} 个.<br />
          请考虑更改 世界配置器 中的气候设置`;
        $("#alert").dialog({
          resizable: false,
          title: "极端气候警告",
