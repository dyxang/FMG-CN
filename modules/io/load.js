...
function quickLoad() {
  ldb.get("lastMap", blob => {
    if (blob) {
      ...
      tip("没有存储地图。请先将地图保存到存储区", true, "error", 2000);
...
function showUploadMessage(type, mapData, mapVersion) {
  ...
  if (type === "invalid") {
    ...
    message = `要加载的地图版本 (${mapVersion}) 比现在的版本更新.<br>请加载相应版本的文件`;
    title = "新文件";
    canBeLoaded = false;
  ...
  try {
    ...
    void (function checkDataIntegrity() {
      ...
      invalidCultures.forEach(c => {
        ...
        invalidCells.forEach(i => (cells.province[i] = 0));
        ERROR && console.error("数据完整性检查. Invalid culture", c, "被分配到各个单元格", invalidCells);
      });
      ...
      pack.provinces.forEach(p => {
        ...
        ERROR && console.error("数据完整性检查. 省", p.i, "链接到已删除状态", p.state);
